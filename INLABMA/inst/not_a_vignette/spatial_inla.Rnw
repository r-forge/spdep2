...
<<echo=FALSE>>=
#TRUE: Run models
#FALSE: Download results from author's website for Leroux, CARBayes and spatial 
#	econometrics models
runleroux <- TRUE
runcarbayes <- TRUE
runsem <- TRUE
...
library("maptools")
boston<-readShapePoly(system.file("etc/shapes/boston_tracts.shp", 
   package="spdep")[1])
...
<<results=hide>>=
library("rgdal")
boston<-readOGR( system.file("etc/shapes",package="spdep")[1], 
   "boston_tracts")
@
\noindent
...
<<>>=
library("spdep")
bostonadj<-poly2nb(boston, queen=FALSE)
@
\noindent
...
#This will the call to INLA to be split in several lines according to
#options()$width and fit in the page
m1$call<-strwrap(m1$call)
@
...
par(mfrow=c(4,4))
for(i in names(m1$marginals.fixed) )
{
	plot(m1$marginals.fixed[[i]], main=i, type="l")

}
plot(m1$marginals.hyperpar[[1]], #xlim=c(0, 2000), 
   type="l",    main="Precision error")
plot(m1$marginals.hyperpar[[2]], #xlim=c(0,50), 
   type="l", main="Precision spatial effects")
@
<<eval=FALSE, echo=FALSE, fig=TRUE, width=8, height=8>>=
par(mfrow=c(2,1))
plot(m1$marginals.hyperpar[[1]], #xlim=c(0, 2000), 
   type="l", main="Precision of error term")
plot(m1$marginals.hyperpar[[2]], #xlim=c(0,50), 
   type="l", main="Precision of spatial random effects")
...
<<keep.source=FALSE>>=
#For some reason, having 'logCMEDV' and 'FTDlogCMEDV' in 'boston' clashes with
#model fitting when log(CMEDV) is used in the response. We have renamed
#these columns to using 'LOG' and not 'log'
#
boston$LOGCMEDV<-log(boston$CMEDV)
boston$FTDLOGCMEDV<-m1$summary.fitted[,"mean"]
@
\noindent
...
spplot(boston, c("LOGCMEDV", "FTDLOGCMEDV"),
...
\begin{figure}[h!]
\begin{center}
<<fig=TRUE, results=hide, echo=FALSE, results=hide>>=
library("RColorBrewer")
print(spplot(boston, c("LOGCMEDV", "FTDLOGCMEDV"),
   cuts=8, col.regions=brewer.pal(9,"Blues"),
   names.attr=c("Observed CMEDV", "Predicted CMEDV")
))
@
\end{center}
...
<<keep.source=FALSE>>=
#Reproject
bostonf<-spTransform(boston, CRS("+proj=longlat +datum=WGS84"))
#Google Maps
library("ggmap")

bostonf <- fortify(bostonf, region="TRACT")
#Add data
idx<-match(bostonf$id, as.character(boston$TRACT) )
bostonf$LOGCMEDV<-boston$LOGCMEDV[idx]
@
\noindent
...
bb <- center_bbox(-71, 42.3, 700, 700)
#bb<-as.vector(bbox(boston))
#bb <- corner_bbox(left=bb[1], bottom=bb[2], right=bb[3], top=bb[4])
...
japnb <- poly2nb(polygrdjap, queen=FALSE, row.names=1:100)

adjpine <- nb2mat(japnb, style = "B")
adjpine<-as(adjpine, "dgTMatrix")
@
\noindent
...
japgrd$INLALA <-  japinlala$summary.fitted.values[, "mean"]
@
...
<<echo=TRUE>>=
fpprw2d  <-  Ntrees ~ 1 + f(japgrd$SPIDX, model = "rw2d", nrow=10, ncol=10,
  hyper=list(prec=list(prior="loggamma", param=c(0.001, 0.001) )) )

japinlalarw2d <- inla(fpprw2d,
   family = "poisson", data = as.data.frame(japgrd),
   control.compute = list(dic = TRUE),
   control.inla=list(tolerance=1e-20, h=1e-8),
   control.predictor = list(compute = TRUE)
...
japgrd$INLALARW2D <-  japinlalarw2d$summary.fitted.values[, "mean"]
@
...
print(spplot(japgrd, c("Ntrees", "INLALA", "INLALARW2D"),
   #at=c(0.6, 0.9801, 1.055, 1.087, 1.125, 1.6),
  cuts=10,
   col.regions=gray.colors(10+1, 0.9, 0.4),
   names.attr=c("DATA", "INLA-BYM", "INLA-RW2D"),
...
<<keep.source=TRUE>>=
formula1 <-  ratioag ~ 1 + f(spgrd$IDX, model = "matern2d", 
   nrow = Nrow, ncol = Ncol,
   hyper = list(range = list(initial = log(sqrt(8)/.5), fixed = TRUE),
            prec = list(initial = log(1), fixed = TRUE)))
                

rglinlala <- inla(formula1, family = "poisson",
   control.predictor = list(compute = TRUE),
   control.compute = list(dic = TRUE),
   data = as.data.frame(spgrd) )

spgrd$INLALA <- rglinlala$summary.fitted.values[,"mean"]
@
...
<<keep.source=TRUE>>=
formularw2d <-  ratioag ~ 1 + f(spgrd$IDX, model = "rw2d",
   nrow = Nrow, ncol = Ncol,
      hyper=list(prec=list(prior="loggamma", param=c(1, 1) )) )

rglinlalarw2d <- inla(formularw2d, family = "poisson",
   control.predictor = list(compute = TRUE),
   control.compute = list(dic = TRUE),
   data = as.data.frame(spgrd) )

spgrd$INLALARW2D <- rglinlalarw2d$summary.fitted.values[,"mean"]
@
...
print(spplot(spgrd, c("ratioag", "INLALA", "INLALARW2D"),
   at=seq(0, 12, length.out=10),
   col.regions=gray.colors(10, 0.9, 0.4),
   names.attr=c("DATA", "INLA-MATERN2D", "INLA-RW2D"),
   par.strip.text=list(cex=1)
))
...
<<>>=
library("R2BayesX")
bayesxadj<-nb2gra(japnb)

japbayesx<-bayesx(Ntrees ~ 1+sx(SPIDX, bs="re")+sx(SPIDX,bs="spatial", 
...
<<>>=
japgrd$BAYESX<-japbayesx$fitted.values[order(japbayesx$bayesx.setup$order),
   "mu"]
@
\noindent
...
\begin{center}
<<echo=FALSE, fig=TRUE, width=8, height=4>>=
print(spplot(japgrd, c("Ntrees", "INLALA", "INLALARW2D", "BAYESX"),
   #at=c(0.6, 0.9801, 1.055, 1.087, 1.125, 1.6),
  cuts=10,
   col.regions=gray.colors(10+1, 0.9, 0.4),
   names.attr=c("DATA", "INLA-BYM", "INLA-RW2D", "BAYESX"),
   par.strip.text=list(cex=1),
   sp.layout=list(list("sp.points", SpatialPoints(coords.ppp(japanesepines))))
))
@
...
boston.matB <- listw2mat( nb2listw(bostonadj, style = "B") )
...
leroux.inla(form2, d = as.data.frame(boston), W = boston.matB, lambda = lambda,
...
save(file="lerouxmodels.RData", list=c("lerouxmodels"))
	...
	download.file("http://www.uclm.es/profesorado/vgomez/JSS1171/lerouxmodels.jpg", "lerouxmodels.RData")
...
<<>>=
bmaleroux <- INLABMA(lerouxmodels, rlambda, 0, impacts=FALSE)
@
<<echo=FALSE, eval=FALSE>>=
...
@
<<carbayes, eval=FALSE>>=
bostondf<-as.data.frame(boston)
attach(bostondf)
lcarbayes <-  lerouxCAR.re (form2, W = boston.matB, family="gaussian",
...
save(file="lcarbayes.RData", list=c("lcarbayes"))
	...
	if(!file.exists("lcarbayes.RData"))
		download.file("http://www.uclm.es/profesorado/vgomez/JSS1171/lcarbayes.jpg", "lcarbayes.RData")
...
#Fitted values
plot(lcarbayes$fitted.values[,1], bmaleroux$summary.fitted.values[,"mean"],
...
leroux2 <- inla(form3,  data=as.data.frame(boston), family = "gaussian",
#    control.family = list(hyper = zero.variance),
     control.predictor=list(compute=TRUE),
                        control.compute=list(dic=TRUE, cpo=TRUE),
                        control.inla=list(print.joint.hyper=TRUE)
...
save(file="semmodels.RData", list=c("semmodels"))
	...
	if(!file.exists("semmodels.RData"))
		download.file("http://www.uclm.es/profesorado/vgomez/JSS1171/semmodels.jpg", "semmodels.RData")
...
<<>>=
mmatrix <- model.matrix(fform, as.data.frame(boston))
...
save(file="slmmodels.RData", list=c("slmmodels"))
	...
	if(!file.exists("slmmodels.RData"))
		download.file("http://www.uclm.es/profesorado/vgomez/JSS1171/slmmodels.jpg", "slmmodels.RData")
...
<<>>=
bmaslm <- INLABMA(slmmodels, rrho2, 0 , impacts = FALSE)
@
...
save(file="sdmmodels.RData", list=c("sdmmodels"))
	...
	if(!file.exists("sdmmodels.RData"))
		download.file("http://www.uclm.es/profesorado/vgomez/JSS1171/sdmmodels.jpg", "sdmmodels.RData")
...
<<>>=
bmasdm <- INLABMA(sdmmodels, rrho3, 0 , impacts = FALSE)
@
