...
defined in variable \code{ID}.

\begin{Schunk}
\begin{Sinput}
R> library("INLA")
R> boston.c$ID <- 1 : nrow(boston.c)
R> form <- log(CMEDV) ~ CRIM + ZN + INDUS + CHAS + I(NOX^2) + I(RM^2) +  AGE + 
...
\begin{figure}[h]
\begin{center}
\includegraphics{spatial_inla-005}
\end{center}
...
After the creation of the grid, we have used function \code{overlay()} on
...
R> idxpp <- overlay(SpatialPoints(japd), polygrdjap)
...
\begin{figure}[h]
\begin{center}
\includegraphics{spatial_inla-018}
...
\begin{figure}[h]
\begin{center}
\includegraphics{spatial_inla-049}
\end{center}
