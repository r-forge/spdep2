...
\begin{Schunk}
\begin{Sinput}
> fform <- log(CMEDV) ~ CRIM + ZN + INDUS + CHAS + I(NOX^2) + I(RM^2) + 
+     AGE + log(DIS) + log(RAD) + TAX + PTRATIO + B + log(LSTAT)
...
+         family = "gaussian", impacts = FALSE, control.family = list(hyper = zero.variance), 
+         control.predictor = list(compute = TRUE), control.compute = list(dic = TRUE, 
+             cpo = TRUE), control.inla = list(print.joint.hyper = TRUE), 
+         verbose = FALSE)
+ })
...
\begin{Schunk}
\begin{Sinput}
> slmmodels = mclapply(rrho2, function(rho) {
+     slm.inla(form, d = boston.c, W = boston.mat, rho = rho, mmatrix = mmatrix, 
+         family = "gaussian", impacts = TRUE, control.data = list(hyper = zero.variance), 
+         control.predictor = list(compute = TRUE), control.compute = list(dic = TRUE, 
+             cpo = TRUE), control.inla = list(print.joint.hyper = TRUE), 
+         verbose = FALSE)
+ })
...
\begin{Schunk}
\begin{Sinput}
> sdmmodels = mclapply(rrho3, function(rho) {
