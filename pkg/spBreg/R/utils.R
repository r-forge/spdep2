...
#sdm_marginal2
rho_marginal2 <- function(detval,e0,ed,epe0,eped,epe0d,nobs,nvar, logdetx, a1,a2,c_beta,TI,xs,ys,sige,W){#VIRGILIO: W is not needed
	n = nrow(detval)
	nmk = (nobs-nvar)/2
	bprior = dbeta(detval[,1],a1,a2)
	C = log(bprior) + lgamma(nmk) - nmk*log(2*pi)
	iota = matrix(1,n,1)
	z = as.numeric(epe0)*iota - 2*detval[,1]*epe0d[1,1] + detval[,1]*detval[,1]*eped[,1]
	Q1 = matrix(0,n,1);
	Q2 = matrix(0,n,1);
	xpxi = solve(t(xs)%*%xs);
	sTI = sige*TI;
	...
	C = C - 0.5*logdetx;
          for (i in 1:n){
           ...
           Q1[i,1] = (t(c_beta - beta)%*%sTI%*%(c_beta - beta))[1,1]
           Q2[i,1] = (t(bhat - beta)%*%(t(xs)%*%xs)%*%(bhat - beta))[1,1]
          }
